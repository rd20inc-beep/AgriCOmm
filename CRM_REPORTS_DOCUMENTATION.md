...
**Per-report detail (representative — Purchase Ledger, the most developed):**
- **Columns:** `['Date','Lot','Supplier','Rice Type','Variety/Grade','Qty (MT)','kg','Per kg','Katta','Value (PKR)','Payment']`.
- **Child rows (lifecycle):** Remaining on hand → Milled into `<batch>` → Finished/By-product sold `(N% share)` to `<customer>` via `<sale>` → Sold (raw) to `<customer>`.
- **RefLinks:** Lot→`/lot-inventory/:id`, Batch→`/milling/:id`, Supplier/Customer→statements, Sale→`/local-sales/:id`, Order→`/export/:id`.
- **Actions:** Print; expand groups; open related records. **No CSV/XLSX, no schedule, no email.**
- **Permissions/Performance/Limitations:** as §1; detail rows capped (200–500), no pagination; amounts exact (good); print CSS only (no native PDF/Excel).

**Sample (Purchase Ledger):** `27 Jun · AAB-1121BASM · A.A Broker · 1121 Basmati White · 15 MT · 15,000 kg · Rs 188 · 300 · Rs 28.20L · Pending` → *Remaining 0 · Milled into M-001 (15 MT) · Finished sold LS-0001 → Aly Shah 2 MT Rs 5.34L (100% share)*.

**Workflow:** month-end financials (P&L/cashflow + AI narrative for the owner), supplier/customer statements, stock valuation snapshots, audit review.
