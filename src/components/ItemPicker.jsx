 ...
 * addToast, placeholder.
 */
export default function ItemPicker({ label, value, onChange, items = [], onItemAdded, addToast, placeholder = 'Search item…' }) {
  const [search, setSearch] = useState('');
  const [open, setOpen] = useState(false);
  const [adding, setAdding] = useState(false);
  const [saving, setSaving] = useState(false);
  const [draft, setDraft] = useState({ name: '', code: '', category: 'packaging', unit: 'piece', reorder_level: '' });
  ...
  async function saveDraft() {
    ...
    try {
      const res = await millStoreApi.createItem({
        code, name, category: draft.category, unit: draft.unit,
      ...
      setDraft({ name: '', code: '', category: 'packaging', unit: 'piece', reorder_level: '' });
      ...
      {adding && (
        <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-lg space-y-2">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button type="button" onClick={saveDraft} disabled={saving || !draft.name.trim() || !draft.code.trim()}
