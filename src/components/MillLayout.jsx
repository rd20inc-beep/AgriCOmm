...
import {
  ...
  ChevronsLeft, ChevronsRight, Sun, Moon, Truck, Printer, Users, FileText,
} from 'lucide-react';
...
export default function MillLayout({ children }) {
  ...
  const handleSearch = (e) => {
    if (e.key === 'Enter' && searchQuery.trim()) {
      ...
      if (query.toUpperCase().startsWith('M-')) navigate(`/milling/${query.toUpperCase()}`);
      else navigate('/milling');
      setSearchQuery('');
    }
  };
        ...
        <div className="flex items-center gap-2.5 px-4 h-14 border-b border-white/[0.08] flex-shrink-0">
          ...
          {!sidebarCollapsed && (
            <div className="min-w-0">
              <span className="text-[14px] font-bold text-white tracking-wide block leading-tight">MILL OPERATIONS</span>
            ...
            {userMenuOpen && (
              <div className="absolute right-0 mt-2 w-52 bg-white rounded-xl shadow-xl border border-gray-200 py-1 z-50 overflow-hidden">
                <div className="px-4 py-2.5 border-b border-gray-100">
                  ...
                  <p className="text-[10px] text-blue-600 uppercase tracking-wider font-semibold mt-1">{user?.role || 'Mill Manager'}</p>
                </div>
