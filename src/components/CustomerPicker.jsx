...
/**
 * Searchable customer/client picker with an inline "+ Add new" that quick-adds a
 * customer (pending admin approval for non-admins). Mirrors SupplierPicker but
 * self-fetches the customer list (parents rarely preload customers).
 *
 * Props: label (node), value (customer_id string), onChange(id), placeholder,
 * addToast, clearable (default false), customers[] (optional preloaded list),
 * onCreated(customer).
 */
...
export default function CustomerPicker({
  ...
  const merged = useMemo(() => {
    const seen = new Set();
    const out = [];
    for (const c of [...local, ...customers, ...fetched]) {
  ...
  useEffect(() => {
    ...
    const onClick = (e) => { if (!e.target.closest('[data-customer-picker]')) setOpen(false); };
  ...
  async function saveDraft() {
    ...
    try {
      ...
      if (!customer?.id) throw new Error('Server did not return the new client');
      const normalized = { id: customer.id, name: customer.name, is_active: true, approval_status: customer.approval_status };
      setLocal(prev => [normalized, ...prev]);
      ...
      const pending = customer.approval_status === 'pending';
      addToast?.(pending ? `Client “${name}” added — pending admin approval` : `Client “${name}” added`, pending ? 'info' : 'success');
    } catch (err) {
      addToast?.(err?.response?.data?.message || err.message || 'Failed to add client', 'error');
    } finally {
      setSaving(false);
    }
  }
