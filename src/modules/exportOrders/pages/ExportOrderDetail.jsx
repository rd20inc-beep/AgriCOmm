    ...
    <div className="space-y-6">
      ...
      {order.status === 'In Milling' && order.millingOrderId && (
        <div className="bg-blue-50 border border-blue-300 rounded-xl p-4 flex items-center justify-between">
          <div>
            <p className="text-sm font-semibold text-blue-800">In Milling</p>
            <p className="text-xs text-blue-600">Milling batch is being processed. Waiting for yield recording and quality approval.</p>
          </div>
          <button onClick={() => navigate(`/milling/${order.millingOrderId}`)} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">View Batch</button>
        </div>
      )}
      ...
      {order.status === 'Ready to Ship' && (
        <div className="bg-sky-50 border border-sky-300 rounded-xl p-4 flex items-center justify-between">
          <div>
            <p className="text-sm font-semibold text-sky-800">Ready to Ship</p>
            <p className="text-xs text-sky-600">Add vessel, booking number, and container details to arrange shipment.</p>
          </div>
          <button onClick={openShipmentModal} className="px-4 py-2 bg-sky-600 text-white rounded-lg text-sm font-medium hover:bg-sky-700">Update Shipment</button>
        </div>
      )}
      {order.status === 'Shipped' && (
        <div className="bg-cyan-50 border border-cyan-300 rounded-xl p-4">
          <p className="text-sm font-semibold text-cyan-800">Shipped — In Transit</p>
          <p className="text-xs text-cyan-600">{order.vesselName ? `Vessel: ${order.vesselName}` : 'Vessel details pending'} {order.blNumber ? `| B/L: ${order.blNumber}` : ''}</p>
        </div>
      )}
      {order.status === 'Arrived' && canCloseOrder && (
