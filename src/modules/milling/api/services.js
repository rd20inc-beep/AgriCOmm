import api from '../../../api/client';

// Service Milling billing (Phase A2).
export const serviceMillingApi = {
  listClients: (params) => api.get('/api/service-milling/clients', params),
  createClient: (data) => api.post('/api/service-milling/clients', data),
  listInvoices: () => api.get('/api/service-milling/invoices'),
  getInvoice: (id) => api.get(`/api/service-milling/invoices/${id}`),
  createInvoice: (data) => api.post('/api/service-milling/invoices', data),
  recordPayment: (id, data) => api.post(`/api/service-milling/invoices/${id}/payments`, data),
  voidInvoice: (id) => api.post(`/api/service-milling/invoices/${id}/void`),
  // Dispatch — hand client-owned finished/by-product stock back to the client.
  getDispatchSummary: (batchId) => api.get(`/api/service-milling/batches/${batchId}/dispatch-summary`),
  createDispatch: (batchId, data) => api.post(`/api/service-milling/batches/${batchId}/dispatches`, data),
  deleteDispatch: (dispatchId) => api.delete(`/api/service-milling/dispatches/${dispatchId}`),
};

export const millingApi = {
  listBatches: (params) => api.get('/api/milling/batches', params),
  listServiceBatches: () => api.get('/api/milling/service-batches'),
  getBatch: (id) => api.get(`/api/milling/batches/${id}`),
  sourceLots: (id) => api.get(`/api/milling/batches/${id}/source-lots`),
  cashFlow: (params) => api.get('/api/milling/cash-flow', params),
  createBatch: (data) => api.post('/api/milling/batches', data),
  updateBatch: (id, data) => api.put(`/api/milling/batches/${id}`, data),
  saveQuality: (id, data) => api.post(`/api/milling/batches/${id}/quality`, data),
  recordYield: (id, data) => api.post(`/api/milling/batches/${id}/yield`, data),
  setMilledQty: (id, data) => api.put(`/api/milling/batches/${id}/milled-qty`, data),
  addCost: (id, data) => api.post(`/api/milling/batches/${id}/costs`, data),
  addVehicle: (id, data) => api.post(`/api/milling/batches/${id}/vehicles`, data),
  updateVehicle: (id, vehicleId, data) => api.put(`/api/milling/batches/${id}/vehicles/${vehicleId}`, data),
  deleteVehicle: (id, vehicleId) => api.delete(`/api/milling/batches/${id}/vehicles/${vehicleId}`),
  receiveRice: (data) => api.post('/api/milling/rice-receipts', data),
  listRicePurchases: (params) => api.get('/api/milling/rice-purchases', params),
  deleteBatch: (id) => api.delete(`/api/milling/batches/${id}`),
  getLastPrices: () => api.get('/api/milling/last-prices'),
  confirmPrices: (id, data) => api.put(`/api/milling/batches/${id}/prices`, data),
  listExpenses: (params) => api.get('/api/milling/expenses', params),
  createExpense: (data) => api.post('/api/milling/expenses', data),
  listRecurringExpenses: () => api.get('/api/milling/expenses/recurring'),
  materializeRecurring: (id, data) => api.post(`/api/milling/expenses/recurring/${id}/materialize`, data),
  runDueRecurring: () => api.post('/api/milling/expenses/recurring/run-due', {}),
  listWorkers: (params) => api.get('/api/milling/workers', params),
  createWorker: (data) => api.post('/api/milling/workers', data),
  updateWorker: (id, data) => api.put(`/api/milling/workers/${id}`, data),
  deleteWorker: (id) => api.delete(`/api/milling/workers/${id}`),
  setWorkerPortalPin: (id, data) => api.post(`/api/milling/workers/${id}/portal-pin`, data),
  listWorkerRequests: (params) => api.get('/api/milling/worker-requests', params),
  workerRequestsCount: () => api.get('/api/milling/worker-requests/count'),
  resolveWorkerRequest: (id, data) => api.post(`/api/milling/worker-requests/${id}/resolve`, data),
  listLeaveTypes: () => api.get('/api/milling/payroll/leave-types'),
  createLeaveType: (data) => api.post('/api/milling/payroll/leave-types', data),
  updateLeaveType: (id, data) => api.put(`/api/milling/payroll/leave-types/${id}`, data),
  deleteLeaveType: (id) => api.delete(`/api/milling/payroll/leave-types/${id}`),
  listLeaveRequests: (params) => api.get('/api/milling/payroll/leave-requests', params),
  leaveRequestsCount: () => api.get('/api/milling/payroll/leave-requests/count'),
  createLeaveRequest: (data) => api.post('/api/milling/payroll/leave-requests', data),
  approveLeaveRequest: (id) => api.post(`/api/milling/payroll/leave-requests/${id}/approve`),
  rejectLeaveRequest: (id) => api.post(`/api/milling/payroll/leave-requests/${id}/reject`),
  deleteLeaveRequest: (id) => api.delete(`/api/milling/payroll/leave-requests/${id}`),
  leaveBalances: (params) => api.get('/api/milling/payroll/leave-balances', params),
  computeFinalSettlement: (workerId) => api.get(`/api/milling/payroll/final-settlement/${workerId}`),
  finalizeSettlement: (workerId, data) => api.post(`/api/milling/payroll/final-settlement/${workerId}`, data),
  listFinalSettlements: () => api.get('/api/milling/payroll/final-settlements'),
  reverseFinalSettlement: (id) => api.delete(`/api/milling/payroll/final-settlements/${id}`),
  payrollAudit: (params) => api.get('/api/milling/payroll/audit', params),
  reviseSalary: (workerId, data) => api.post(`/api/milling/workers/${workerId}/salary-revision`, data),
  listSalaryRevisions: (workerId) => api.get(`/api/milling/workers/${workerId}/salary-revisions`),
  listWorkerAdvances: (id) => api.get(`/api/milling/workers/${id}/advances`),
  workerLedger: (id) => api.get(`/api/milling/workers/${id}/ledger`),
  createWorkerAdvance: (id, data) => api.post(`/api/milling/workers/${id}/advances`, data),
  deleteWorkerAdvance: (id) => api.delete(`/api/milling/advances/${id}`),
  advanceLedger: (id) => api.get(`/api/milling/advances/${id}/ledger`),
  pendingAdvances: () => api.get('/api/milling/advances/pending'),
  approveAdvance: (id, data) => api.post(`/api/milling/advances/${id}/approve`, data || {}),
  rejectAdvance: (id, data) => api.post(`/api/milling/advances/${id}/reject`, data || {}),
  payAdvance: (id, data) => api.post(`/api/milling/advances/${id}/pay`, data || {}),
  listWorkerAdjustments: (id) => api.get(`/api/milling/workers/${id}/adjustments`),
  createWorkerAdjustment: (id, data) => api.post(`/api/milling/workers/${id}/adjustments`, data),
  deleteWorkerAdjustment: (id) => api.delete(`/api/milling/adjustments/${id}`),
  listAttendance: (params) => api.get('/api/milling/attendance', params),
  recordAttendance: (data) => api.post('/api/milling/attendance', data),
  bulkAttendance: (data) => api.post('/api/milling/attendance/bulk', data),
  importAttendance: (data) => api.post('/api/milling/attendance/import', data),
  attendanceHolidays: (params) => api.get('/api/milling/attendance/holidays', params),
  payrollSummary: (params) => api.get('/api/milling/payroll/summary', params),
  postPayrollRun: (data) => api.post('/api/milling/payroll/run', data),
  listPayrollRuns: (params) => api.get('/api/milling/payroll/runs', params),
  payrollReport: (params) => api.get('/api/milling/payroll/report', params),
  getPayrollRun: (id) => api.get(`/api/milling/payroll/runs/${id}`),
  approvePayrollRun: (id) => api.post(`/api/milling/payroll/runs/${id}/approve`),
  payPayrollRun: (id, lineIds) => api.post(`/api/milling/payroll/runs/${id}/pay`, (lineIds && lineIds.length) ? { line_ids: lineIds } : {}),
  accruePayrollRun: (id) => api.post(`/api/milling/payroll/runs/${id}/accrue`),
  settlePayrollRun: (id) => api.post(`/api/milling/payroll/runs/${id}/settle`),
  listStatutoryDeductions: () => api.get('/api/milling/payroll/statutory-deductions'),
  createStatutoryDeduction: (data) => api.post('/api/milling/payroll/statutory-deductions', data),
  updateStatutoryDeduction: (id, data) => api.put(`/api/milling/payroll/statutory-deductions/${id}`, data),
  deleteStatutoryDeduction: (id) => api.delete(`/api/milling/payroll/statutory-deductions/${id}`),
  listStatutoryLiabilities: (params) => api.get('/api/milling/payroll/statutory-liabilities', params),
  listStatutoryRemittances: (params) => api.get('/api/milling/payroll/statutory-remittances', params),
  createStatutoryRemittance: (data) => api.post('/api/milling/payroll/statutory-remittances', data),
  deleteStatutoryRemittance: (id) => api.delete(`/api/milling/payroll/statutory-remittances/${id}`),
  taxStatement: (params) => api.get('/api/milling/payroll/tax-statement', params),
  voidPayrollRun: (id, reason) => api.post(`/api/milling/payroll/runs/${id}/void`, { reason }),
  deletePayrollRun: (id) => api.delete(`/api/milling/payroll/runs/${id}`),
  getPayrollSchedule: () => api.get('/api/milling/payroll/schedule'),
  savePayrollSchedule: (data) => api.put('/api/milling/payroll/schedule', data),
  runPayrollNow: (data) => api.post('/api/milling/payroll/schedule/run-now', data || {}),
  listUtilities: (params) => api.get('/api/milling/utilities', params),
  recordUtility: (data) => api.post('/api/milling/utilities', data),
  listMills: (params) => api.get('/api/milling/mills', params),
  createMill: (data) => api.post('/api/milling/mills', data),
  updateMill: (id, data) => api.put(`/api/milling/mills/${id}`, data),
  deleteMill: (id) => api.delete(`/api/milling/mills/${id}`),
  listPlans: (params) => api.get('/api/milling/plans', params),
  createPlan: (data) => api.post('/api/milling/plans', data),
  recordDowntime: (data) => api.post('/api/milling/downtime', data),
  approveBatch: (id, body = {}) => api.put(`/api/milling/batches/${id}/approve`, body),
  rejectBatch: (id, data) => api.put(`/api/milling/batches/${id}/reject`, data),
};
