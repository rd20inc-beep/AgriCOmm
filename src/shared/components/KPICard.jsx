...
export default function KPICard({ icon: Icon, title, value, subtitle, trend, color = 'blue', lines }) {
            ...
            <div className="mt-1 space-y-0.5">
              {lines.map((line, i) => (
                <div key={i} className="flex items-baseline justify-between gap-2">
                  ...
                  <span className="text-sm font-bold text-gray-900 tabular-nums whitespace-nowrap">{line.value}</span>
                </div>
              ))}
            </div>
          ) : (
